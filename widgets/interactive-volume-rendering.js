...
function render({ model, el }) {
  ...
  loadVolume(dataUrl, metaUrl).then(({ vol, nx, ny, nz, meta }) => {
      ...
      <div class="${id}-row">
        ...
        <div class="${id}-controls">
          ...
          <div class="${id}-ctrl">
            ...
            <input class="${id}-slider ${id}-vmin" type="range" min="0" max="254" step="1" value="64"/>
            <div class="${id}-slider-row"><span>min =</span><span class="${id}-vmin-val">64</span></div>
          </div>
          <div class="${id}-ctrl">
    ...
    const alphaSlider = $(`.${id}-alpha`);
    const alphaVal = $(`.${id}-alpha-val`);
    const vminSlider = $(`.${id}-vmin`);
