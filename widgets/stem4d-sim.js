...
function render({ model, el }) {
  ...
  setTimeout(() => {
    ...
    // Use pointer events with setPointerCapture so the drag persists when
    // the cursor leaves the canvas — no document-level listeners needed
    // (shadow-DOM safe).
    ...
    const _stem4dEndDrag = (e) => {
      isDragging = false;
      try { canvas.releasePointerCapture(e.pointerId); } catch (_) {}
    };
    canvas.addEventListener("pointerup", _stem4dEndDrag);
    canvas.addEventListener("pointercancel", _stem4dEndDrag);
