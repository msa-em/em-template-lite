...
def main() -> int:
    ...
    helper_lines = [
        ...
        "# Silence matplotlib's first-import font-cache chatter (mpl issue #23326).\n",
        "# Pyodide pre-imports matplotlib at kernel boot so just raising the log\n",
        "# level can be too late; disable the loggers entirely + filter the warning\n",
        "# string + intercept the stderr write as a last resort.\n",
        "warnings.filterwarnings('ignore', message='.*Matplotlib is building the font cache.*')\n",
        "for _lname in ('matplotlib', 'matplotlib.font_manager'):\n",
        "    _l = logging.getLogger(_lname); _l.setLevel(logging.ERROR); _l.disabled = True\n",
        "_orig_stderr_write = sys.stderr.write\n",
        "def _filtered_stderr_write(s, _orig=_orig_stderr_write):\n",
        "    if 'building the font cache' in s: return len(s)\n",
        "    return _orig(s)\n",
