...
def main() -> int:
    ...
    helper_lines = [
        ...
        "    # own location, walking up to the `lite/` segment. This avoids hardcoding\n",
        "    # the deploy path so the same notebook works on forks / custom domains.\n",
        "    import js\n",
        "    from pyodide.http import pyfetch\n",
        "    worker_url = str(js.self.location.href)\n",
        "    idx = worker_url.find('/lite/')\n",
        "    if idx < 0:\n",
        "        raise FileNotFoundError(f'{path}: could not locate JupyterLite root in worker URL {worker_url!r}')\n",
        "    url = worker_url[:idx + len('/lite/')] + 'files/' + path\n",
        "    r = await pyfetch(url)\n",
        "    if r.status != 200:\n",
